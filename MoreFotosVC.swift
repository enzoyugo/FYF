...
class MoreFotosVC: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    
    
    @IBOutlet weak var clothesLbl: UILabel!
    @IBOutlet weak var photosColl: UICollectionView!
    var clothes : Clothes?
    ...
    override func viewDidLoad() {
        ...
        photosColl?.register(UINib(nibName: "MasFotosCollViewCell", bundle: nil), forCellWithReuseIdentifier: "MasFotosCollViewCell")
    ...
    func onDocumentAdded(change: DocumentChange, category: Photos){
        ...
        photosColl?.insertItems(at: [IndexPath(item: newIndex, section: 0)])
    ...
    func onDocumentModified(change: DocumentChange, category: Photos){
        if change.newIndex == change.oldIndex {
            ...
            photosColl?.reloadItems(at: [IndexPath(item: index, section: 0)])
            ...
            photosColl?.moveItem(at: IndexPath(item: oldIndex, section: 0), to: IndexPath(item: newIndex, section: 0))
        }
    }
    func onDocumentRemoved(change: DocumentChange){
        let oldIndex = Int(change.oldIndex)
        photo.remove(at: Int(oldIndex))
        photosColl?.deleteItems(at: [IndexPath(row: oldIndex, section: 0)])
    }
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        return CGSize(width: self.view.frame.size.width - 10, height: self.photosColl.frame.size.height - 10)
    }
